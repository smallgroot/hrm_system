...
var custom_contract_ad = $('#custom_contract_list_table').dataTable({
	
	"bDestroy": true,
		"paging": true, /* employee list pagination */
		"ajax": {
			url : main_url+"employees/contract_list/"+$('#user_id').val(),
            type : 'GET'
        },
		"language": {
            "lengthMenu": dt_lengthMenu,
...
// add contract data
$("#user_contract").submit(function(e){
	var fd = new FormData(this);
	var obj = $(this), action = obj.attr('name');
	fd.append("is_ajax", 1);
	fd.append("type", 'add_record');
	...
	$.ajax({
		...
		success: function(JSON)
		{
			if (JSON.error != '') {
				...
				toastr.success(JSON.result);
				custom_contract_ad.api().ajax.reload(function(){ 
					Ladda.stopAll();
					$('input[name="csrf_token"]').val(JSON.csrf_hash);
				}, true);
				jQuery('#user_contract')[0].reset(); // To reset form fields
			}
		},
		error: function() 
		{
...
$("#delete_record").submit(function(e){
	...
	var field_add = '&data=delete_record&type=delete_record&';
	var tb_name = 'custom_contract_list_table';
...
$('.edit-modal-data').on('show.bs.modal', function (event) {
	...
	if(field_tpe == 'contract'){
		var field_add = '&data=user_contract&type=user_contract&';
	}
	var modal = $(this);
	$.ajax({
		url: main_url+'employees/read_contract',
		type: "GET",
		data: 'jd=1'+field_add+'field_id='+field_id+'&uid='+$('#user_id').val(),
		success: function (response) {
			if(response) {
...
// add payment
$('.add_payment_schedule_model').on('show.bs.modal', function (event) {
	var button = $(event.relatedTarget);
	var field_id = button.data('field_id');
	var field_tpe = button.data('field_type');
	if(field_tpe == 'add_payment'){
		var field_add = '&data=user_add_payment&type=user_add_payment&';
	}
	var modal = $(this);
	$.ajax({
		url: main_url+'employees/add_payment_model_open',
		type: "GET",
		data: 'jd=1'+field_add+'field_id='+field_id+'&uid='+$('#user_id').val(),
		success: function (response) {
			if(response) {
				$("#ajax_payment_model").html(response);
			}
		}
	});
});
...
var payment_schedule_ad = $('#payment_schedule_list_table').dataTable({
	
	"bDestroy": true,
		"paging": true, /* employee list pagination */
		"ajax": {
			url : main_url+"employees/payment_schedule_list/"+$('#user_id').val(),
            type : 'GET'
        },
		"language": {
            "lengthMenu": dt_lengthMenu,
...
$("#delete_record").submit(function(e){
	...
	var field_add = '&data=delete_record&type=delete_record&';
	var tb_name = 'payment_schedule_list_table';
	...
	// read edit
	$('.edit_payment_schedule_model').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget);
		var field_id = button.data('field_id');
		var field_tpe = button.data('field_type');
		if(field_tpe == 'payment_schedule'){
			var field_add = '&data=user_payment_schedule&type=user_payment_schedule&';
		}
		var modal = $(this);
		$.ajax({
			url: main_url+'employees/read_payment_schedule',
			type: "GET",
			data: 'jd=1'+field_add+'field_id='+field_id+'&uid='+$('#user_id').val(),
			success: function (response) {
				if(response) {
					$("#edit_ajax_payment_model").html(response);
				}
			}
		});
	});
