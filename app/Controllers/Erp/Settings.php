...
// fund type and contract type start
public function fund_type_list() {
	...
	if($user_info['user_type'] == 'staff'){
		$category = $ConstantsModel->where('company_id',$user_info['company_id'])->where('type','fund_type')->orderBy('constants_id', 'ASC')->findAll();
	} else {
		$category = $ConstantsModel->where('company_id',$usession['sup_user_id'])->where('type','fund_type')->orderBy('constants_id', 'ASC')->findAll();
	}
	$data = array();
	  ...
	  foreach($category as $r) {
		...
		if(in_array('track_type3',staff_role_resource()) || $user_info['user_type'] == 'company') { //edit
			// $edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_edit').'"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light" data-toggle="modal" data-target=".fund_view_modal_data" data-field_id="'. uencode($r['constants_id']) . '"><i class="feather icon-edit"></i></button></span>';
			$edit = '';
		} else {
			$edit = '';
		}
...
public function contract_modality_type_list() {
	...
	$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
	if($user_info['user_type'] == 'staff'){
		$category = $ConstantsModel->where('company_id',$user_info['company_id'])->where('type','contract_type')->orderBy('constants_id', 'ASC')->findAll();
	} else {
		$category = $ConstantsModel->where('company_id',$usession['sup_user_id'])->where('type','contract_type')->orderBy('constants_id', 'ASC')->findAll();
	}
	$data = array();
...
// add
public function add_contract_modality_type() {
...
// fund type and contract type end
