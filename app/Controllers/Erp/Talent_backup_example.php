...
// exit();
		...
		if($user_info['user_type'] == 'staff'){
			...
			if($department_head !=''){
				...
				// echo '<pre>';
				// print_r($user_detail);
				// print_r($user_detail[0]['user_id']);
				// exit();
				
		foreach($user_details as $user_detail){
			// $array_user_ids[] = $user_detail;
		   $array_user_ids[] = $user_detail['user_id'];
		}
		
		// echo $array_user_ids;
		// echo '<pre>';
		// print_r($array_user_ids);
		// exit();
		// $aaaaa = array(187, 180, 182);
		// echo '<pre>';
		// print_r($aaaaa);
		// exit();
		
		
		// $get_users = $UsersModel->whereIn('user_id', $array_user_ids)->findAll();
		
		$get_data = $KpaModel->where('status','1')->whereIn('employee_id',$array_user_ids)->orderBy('performance_appraisal_id', 'ASC')->findAll();
